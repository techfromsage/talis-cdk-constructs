...
AuthPolicy.prototype = (function() { // eslint-disable-line
  /**
  ...
  const addMethod = function addMethod(effect, verb, resource, conditions) {
        if (verb !== '*' && !AuthPolicy.HttpVerb.hasOwnProperty(verb)) { // eslint-disable-line
  ...
  /**
     ...
     */
  const toTitleCase = function toTitleCase(str) {
  ...
  const getEmptyStatement = function getEmptyStatement(effect) {
    const effectTitleCase = toTitleCase(effect);

    const statement = {};
    statement.Action = 'execute-api:Invoke';
    statement.Effect = effectTitleCase;
    statement.Resource = [];
  ...
  return {
    ...
    /**
         ...
         */
    denyAllMethods: function deleteAllMethods() {
...
console.log(`MSL Statement: ${JSON.stringify(policy.policyDocument.Statement)}`);
      return policy;
    },
  };
})();
